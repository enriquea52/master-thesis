...
class AprilDetectSubscriber(Node):
    ...
    def __init__(self):
        ...
        param_descriptor = ParameterDescriptor(description='Stereo Img Topic Left Img.')
        self.declare_parameter('left_img_topic', "", param_descriptor)  
        param_descriptor = ParameterDescriptor(description='Stereo Img Topic Right Img.')
        self.declare_parameter('right_img_topic', "", param_descriptor)  
        ...
        imageL_sub = message_filters.Subscriber(self, CompressedImage, str(self.get_parameter('left_img_topic').value))
        imageR_sub = message_filters.Subscriber(self, CompressedImage, str(self.get_parameter('right_img_topic').value))

        # Synchronize images
        ts = message_filters.ApproximateTimeSynchronizer([imageL_sub, imageR_sub], queue_size=10, slop=0.5)
        ts.registerCallback(self.detect_callback)
    ...
    def detect_callback(self, imageL, imageR):

        # [x, y] = [col, row]
        st = time.time()

        # Use CVBridge to get sensor_msgs/msg/Image and convert it to OpenCV
        img_left = self.br.compressed_imgmsg_to_cv2(imageL)
        img_right = self.br.compressed_imgmsg_to_cv2(imageR)
        ...
        for r in results_left:
            ...
            cv2.circle(img_left, center[:, 0], 5, (0, 0, 255), 5)
        ...
        for r in results_right: 
            ...
            cv2.circle(img_right, center[:, 0], 5, (0, 0, 255), 5)
        ...
        if np.sum(self.tag_present_left) > 0 and np.sum(self.tag_present_right) == 0:
            ...
            self.handle_pose(self.indixes[self.tag_present_left], homography_poses_left, imageL.header.stamp)
            ...
            self.handle_pose(self.indixes[self.tag_present_right], homography_poses_right, imageR.header.stamp)
            ...
            self.handle_pose(self.indixes[associations], poses_opt, imageL.header.stamp)
